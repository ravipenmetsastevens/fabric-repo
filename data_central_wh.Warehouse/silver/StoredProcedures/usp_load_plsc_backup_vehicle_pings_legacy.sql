/*─────────────────────────────────────────────────────────────*
  LOAD  procedure – truncate‑and‑reload
*─────────────────────────────────────────────────────────────*/
CREATE   PROCEDURE [silver].[usp_load_plsc_backup_vehicle_pings_legacy]
AS
BEGIN
    TRUNCATE TABLE [data_central_wh].[silver].[plsc_backup_vehicle_pings_legacy];

    INSERT INTO [data_central_wh].[silver].[plsc_backup_vehicle_pings_legacy]
    (
    ...
    SELECT
        ...
        PHMAC, PHVER
    FROM [data_central_lh].[dbo].[PLSC_BACKUPVEH];
END;