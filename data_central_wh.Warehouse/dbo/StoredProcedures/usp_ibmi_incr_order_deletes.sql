CREATE     PROCEDURE dbo.usp_ibmi_incr_order_deletes
AS
BEGIN
    -- ibmi_incr_order
    UPDATE TGT
    SET    TGT.is_deleted = CASE WHEN L.[ORDER] IS NULL THEN 0 ELSE 1 END
    FROM   silver.ibmi_incr_order AS TGT
    LEFT   JOIN (
    ...
    UPDATE TGT
    SET    TGT.is_deleted = CASE WHEN L.[ORDER] IS NULL THEN 0 ELSE 1 END
    FROM   silver.ibmi_incr_cd_order AS TGT
    LEFT   JOIN (
        SELECT DISTINCT [ORDER]
        FROM data_central_lh.dbo.ibmi_ord_bill_change_log
        WHERE [JOOBJ] = 'ORDER'
    ...
    UPDATE TGT
    SET    TGT.is_deleted = CASE WHEN L.[ORDER] IS NULL THEN 0 ELSE 1 END
    FROM   silver.ibmi_incr_tlb_order AS TGT
    LEFT   JOIN (
        SELECT DISTINCT [ORDER]
        FROM data_central_lh.dbo.ibmi_ord_bill_change_log
        WHERE [JOOBJ] = 'ORDER'
