CREATE PROCEDURE [dbo].[usp_create_fact_ordbill_all]
AS

DELETE FROM gold.fact_ordbill_all

INSERT INTO gold.fact_ordbill_all
SELECT 
	  a.[cd_ordbill_load_number]					AS load_number
	, SUM(a.[cd_ordbill_load_bill_amount])			AS bill_amount

FROM   [silver].[ibmi_cd_ordbill_v2] a 
INNER JOIN [silver].[ibmi_cd_billing] b ON b.[cd_billing_load_number] = a.[cd_ordbill_load_number] 
...
FROM   [silver].[ibmi_tlb_ordbill] a 
INNER JOIN [silver].[ibmi_tlb_billing] b ON b.[tlb_billing_load_number] = a.[tlb_ordbill_load_number] 
...
FROM   [silver].[ibmi_ordbill] a 
INNER JOIN [silver].[ibmi_billing] b ON b.[billing_load_number] = a.[ordbill_load_number] 
